...
- `POST /audit`：仅已封存会话可复核，返回 `HEALTHY` / `DEGRADED` / `REPAIRING`
  ...
  **逐块摘要不符**与**无法定位的整文件摘要不符**。修复进行期间的复核**不排队、
  不等待修复结束**，立即返回 `REPAIRING`，其异常范围与 `repair_started_at`
  冻结自修复开始时刻，同一次修复内连续复核范围绝不漂移。
...
block_index.json # 封存时写入的逐块（长度+SHA-256）可信索引；
                 #   旧会话首次复核成功后补建
repair/plan.json # 先于 source 原子落盘：修复开始时刻冻结的异常块范围；
                 #   并发复核据此立即返回稳定的 REPAIRING，范围不随分块恢复漂移
repair/source   # 仅修复期间存在：已通过长度+整文件摘要双校验的原文件，
                 #   是中断/重启后续修的持久进度标记；收敛后目录被移除
```

修复窗口的可观察性可通过 `REPAIR_CHUNK_DELAY`（秒，默认 `0`）调节：compose
默认设为 `0.25`，让每个异常块替换前留出稳定的 `REPAIRING` 观察窗口，便于
verify 链路并发复核；生产可设为 `0`，语义不变。
...
3. `verify/smoke.py`：对 `http://web:8000` 的纯 stdlib HTTP 全链路冒烟
   （含 audit/repair 的门控与稳定结果），并在共享数据卷上制造已封存缺块，
   在修复进行中并发复核，断言及时返回冻结范围的 `REPAIRING`、范围不漂移、
   收敛后 `HEALTHY` 且原回执字节不变。
