...
- `POST /repair`：提交完整原文件（原始字节作为请求体）。仅当**长度与回执摘要
  ...
  `digest_mismatch`），封存数据零改动（即使修复正在进行，错误文件也会被立即
  拒绝而不排队）。修复中断后重发或服务重启会自动继续直至收敛；原回执与
  `sealed_at` 始终不变。**修复窗口内**的并发复核**立即**（不等待修复结束）
  返回 `REPAIRING`，并稳定报告本次修复开始时冻结的异常范围——即使分块正在
  逐步恢复，同一次修复期间连续复核所见范围也不会漂移；修复完成后后续复核
  收敛为 `HEALTHY`。
...
repair/plan.json # 修复开始、首个分块替换之前冻结的异常块范围快照；
                 #   修复窗口内的并发复核始终按它报告 REPAIRING，范围不漂移
```

短促的状态变迁由进程内锁串行化；耗时的分块替换循环在**每会话修复锁**下
执行、不持有全局锁，因此并发 `/audit` 可以即时返回而不是等到修复结束。
落盘均为「临时文件 → fsync → 原子 rename → fsync 目录」，
...
3. `verify/smoke.py`：对 `http://web:8000` 的纯 stdlib HTTP 全链路冒烟
   （含 audit/repair 的门控与稳定结果，以及修复进行中并发复核即时返回
   REPAIRING、范围不漂移、完成后收敛 HEALTHY 的修复窗口场景）。
